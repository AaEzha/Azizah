...
if(isset($_GET['a']) and isset($_GET['x']))
{
  ...
  else
  {
    if($aksi == md5("start"))
    {
      mysql_query("update internship_registration set STATUS='IN PROGRESS' where GUID='$idinter'");
    ...
    <tr>
      ...
      <td class="text-center">
        <div class="btn-group btn-group-justified" role="group" aria-label="...">
          ...
          <a href="?p=intern&i=<?=$d['GUID'];?>&a=<?=md5('finish');?>&x=<?=md5($d['GUID']);?>" class="btn btn-success btn-sm" title="Finish Internship">Finish</a>
          <?php } ?>
        </div>
      </td>
    </tr>
